...
export function initHero3D() {
  ...
  gsap.from(perspective, {
    opacity: 0,
    scale: compact ? 1.03 : 1.06,
    duration: compact ? 1 : 1.4,
    ease: 'power3.out',
  });
  ...
  // Continuous 3D motion — lighter on compact viewports
  if (cube) {
    gsap.to(cube, {
      rotateY: '+=360',
      duration: compact ? 40 : 28,
      ease: 'none',
      repeat: -1,
    });
